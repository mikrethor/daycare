...
export class SumupServiceImpl implements SumupService{
    ...
    getAllByChildId(idDaycare: number, idChild: number): Observable<Sumups[]> {
        ...
        return this.http.get(this.constantService.API_ENDPOINT + "/daycares/" + idDaycare + "/childs/" + idChild + "/sumups/",this.loginService.getBearerToken2())
    ...
    getOneByChildIdAndDay(idDaycare: number, idChild: number, daySumup: string): Observable<Sumups> {
        ...
        return this.http.get(this.constantService.API_ENDPOINT + "/daycares/" + idDaycare + "/childs/" + idChild + "/sumups/day/" + daySumup,this.loginService.getBearerToken2())
            .catch((error: any) => Observable.throw(error.json().error || 'Server error'));
    }
